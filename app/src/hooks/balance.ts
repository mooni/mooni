...
export const useTradeBalance = (tradeRequest: TradeRequest, multiTradeEstimation: MultiTradeEstimation | null) => {
  ...
  const insufficientBalance = useMemo(() => {
    if ((tradeRequest.tradeExact === TradeExact.OUTPUT && !multiTradeEstimation) || balanceLoading) return false

    const inputAmount = multiTradeEstimation ? multiTradeEstimation.trades[0].inputAmount : tradeRequest.amount
