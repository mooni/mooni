...
function searchFunction(currency: Currency, searchValue: string) {
  ...
  if ((currency as TokenCurrency).address?.toLowerCase() === sl) return true
  else if (currency.symbol.toLowerCase().includes(sl)) return true
  else return false
}

const useSortedList = (currencyList: Currency[], currencyBalances: CurrencyBalances) => {
  return useMemo(() => sortCurrencies(currencyList, currencyBalances), [currencyList, currencyBalances])
}

const useFilteredList = (currencyList: Currency[], searchValue: string) => {
  return useMemo(() => currencyList.filter((c) => searchFunction(c, searchValue)), [currencyList, searchValue])
}
