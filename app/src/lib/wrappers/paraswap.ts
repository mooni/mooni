...
const ParaswapWrapper = {
  ...
  async getRate(tradeRequest: TradeRequest): Promise<DexTrade> {
    ...
    const amountCurrencyObject =
      tradeRequest.tradeExact === TradeExact.INPUT
        ? tradeRequest.inputCurrencyObject
        : tradeRequest.outputCurrencyObject
    ...
    try {
      ...
      const inputAmount = amountToDecimal(dexMetadata.priceRoute.srcAmount, tradeRequest.inputCurrencyObject.decimals)
  ...
  async getSpender(): Promise<string> {
    if (!augustusSpender) {
      ...
      const augustusContract = new ethers.Contract(augustusAddress, AUGUSTUS_ABI, defaultProvider)
  ...
  async buildTx(dexTrade: DexTrade, senderAddress: string): Promise<any> {
    ...
    const txParams = await paraSwapSDK.buildTx(
      srcToken,
      destToken,
      srcAmount,
      destAmount,
