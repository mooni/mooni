...
function ErrorCTA({ action, label, icon }) {
  return (
    <Box mt={2}>
      <RoundButton mode="strong" onClick={action} wide icon={icon} label={label} />
    </Box>
  )
}
...
function ErrorCatcher({ orderErrors }) {
  ...
  if (orderErrors[0].code === 'exceeds_quota') {
    return (
      <>
        <ErrorContent>
          You have reach your limits with this bank account. Please try again with a lower amount or wait 24h so that
          your limits are reset.
        </ErrorContent>
        <Label>Exchange limits</Label>
        <LimitLine>
          <b>Daily</b> {numberWithCommas(dailyLimits['EUR'])} EUR/{numberWithCommas(dailyLimits['CHF'])} CHF
        </LimitLine>
        <LimitLine>
          <b>Annually</b> {numberWithCommas(yearlyLimits['EUR'])} EUR/{numberWithCommas(yearlyLimits['CHF'])} CHF
        </LimitLine>
        <ErrorCTA icon={<IconArrowLeft />} action={() => onRestart(true)} label="Go home" />
      </>
    )
    ...
    return (
      <>
        ...
        <ErrorCTA icon={<IconRotateLeft />} action={onRestart} label="Start over" />
      </>
    )
