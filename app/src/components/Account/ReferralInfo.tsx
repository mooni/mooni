...
export function ReferralBox() {
  ...
  return (
    <Flex direction="column" justify="center" align="center">
      ...
      <Button onClick={copy} leftIcon={hasCopied ? <CheckCircleIcon /> : <CopyIcon />} size="sm" variant="link">
  ...
  return (
    <>
      <Flex direction="column" align="center" mb={2}>
        ...
        <KPIValue>{profitShare ? profitShare.referralTxCount : <LoadingRing mode="half-circle" />}</KPIValue>
      </Flex>
      <ReferralBox />
      <SubContent>
        You can earn money by referring other people to use Mooni ! Any completed order referred by you will make you
        earn {config.referralSharing * 100}% profit sharing.
      ...
      {profitShare && profitShare.referralTxCount > 0 && (
        <SubContent>
          You have accumulated {significantNumbers(profitShare.referralProfit)} ETH so far in profit sharing. Please
          contact support if you want to withdraw that.
        </SubContent>
      )}
    </>
  )
